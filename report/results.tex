...
Still, SafraFT causes only a moderate processing time overheads between %TODO \OutputFileNum{figures/min-ft-processing-time-overhead.txt}\% and \OutputFileNum{figures/max-ft-processing-time-overhead.txt}\%.
...
As for tokens, one can see an increase in total processing times under the presence of faults compared to fault-free runs which is no surprise because more tokens were sent.
