...
\begin{enumerate}
	\item All nodes are passive
	\item No messages are in the channels
	\item Termination is postponed until the last node failure that leads to activity is detected
\end{enumerate}
...
The key observation is that SafraFS and SafraFT behave highly similiar except for networks with 2000 nodes where SafraFS results show much more variance and some extraordinar outliers.
For all other network sizes the differences are small.
SafraFT sends slightly less tokens in average.
...
This does not directly effect the number of tokens. 
...
Furthermore, one sees that SafraFS times roughly linear with the nework size while SafraFT timings show nearly three fold increase for each network size.
...
As mentioned above there is one exception: network sizes of 2000 causes more token to be generated in 5n networks than in networks with 90\% node failure.
...
The size of networks with 90\% node failures is slightly positvely correlated to the overhead of tokens in total and has no influence on the overhead of tokens after termination.
...
An interesting observation is that the difference from fault free processing time to processing time with faults does not necesseraly increase with the network size:
...
\end{table}
 ...
 For network with 90\% node failure the overhead for total time decreases for network sizes between 50 and 1000 nodes and increases to its maximum for 2000 nodes.
