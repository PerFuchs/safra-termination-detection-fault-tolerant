...
\begin{enumerate}
	...
	\item \co{X} removes \co{1} from $CRASHED_t$ (line 3 of \co{ReceivedToken}).
...
\begin{itemize}
	...
	\item Instead of updating all token variables (line 2 of \co{ReceivedToken}), update only the $CRASHED_t$ set in \co{ReceivedToken}\footnote{This means only populating $CRASHED_t$ with the value of the current token, not removing $CRASHED_i$ from it because this functionality moves to \co{HandleToken}}.
