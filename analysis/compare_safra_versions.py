...
def compare_safra_times(configurations):
    ...
    for network_size, (fault_tolerant, fault_sensitive) in configurations.items():
        ...
        show_mean_equality("Safra time", network_size, fault_tolerant.get_safra_times(), fault_sensitive.get_safra_times_after_termination(), True)
    ...
    plotly.offline.plot([get_scatter_graph_data(list(range(5)), x2curve, "safra time FS"), get_scatter_graph_data(list(range(5)), data2, "safra time FS"), get_scatter_graph_data(list(range(5)), data, "safra time FT")], filename='../test.html')
