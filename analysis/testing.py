...
def test_configurations_complete(configurations):
  ...
  for c in configurations:
    ...
    if total_repetition_count != c.expected_repetitions:
      print("  Configurations %i %s is missing %i repetitions from %i" % (
      c.number_of_nodes, c.fault_group, c.expected_repetitions - total_repetition_count, c.expected_repetitions))
      passed = False
...
def test_no_error_files(configurations, summary):
  ...
  passed = True
  for c in configurations:
    if len(c.invalid_repetitions) != 0 and not all(map(manually_reviewed, c.invalid_repetitions)):
      passed = False
...
def manually_reviewed(repetition):
  manual_review_file_name = ['.cmError', '.legitEarlyAnnounce']
  return any(map(lambda name: os.path.isfile('/'.join((repetition.folder, name))), manual_review_file_name))
